...
type Request struct {
	...
	TimerRuUtime    []float32  `protobuf:"fixed32,22,rep,name=timer_ru_utime" json:"timer_ru_utime,omitempty"`
	TimerRuStime    []float32  `protobuf:"fixed32,23,rep,name=timer_ru_stime" json:"timer_ru_stime,omitempty"`
}
...
func (m *Request) MarshalTo(data []byte) (int, error) {
	...
	data[i] = 0x3d
	i++
	i = encodeFixed32Pinba(data, i, uint32(math.Float32bits(m.RequestTime)))
	data[i] = 0x45
	i++
	i = encodeFixed32Pinba(data, i, uint32(math.Float32bits(m.RuUtime)))
	data[i] = 0x4d
	i++
	i = encodeFixed32Pinba(data, i, uint32(math.Float32bits(m.RuStime)))
...
func (m *Request) Unmarshal(data []byte) error {
	...
	for iNdEx < l {
		...
		case 7:
			...
			m.RequestTime = float32(math.Float32frombits(v))
		...
		case 8:
			...
			v |= uint32(data[iNdEx-2]) << 16
			v |= uint32(data[iNdEx-1]) << 24
			m.RuUtime = float32(math.Float32frombits(v))
		...
		case 9:
			...
			v |= uint32(data[iNdEx-2]) << 16
			v |= uint32(data[iNdEx-1]) << 24
			m.RuStime = float32(math.Float32frombits(v))
		...
		default:
			iNdEx = preIndex
			skippy, err := skipPinba(data[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
...
func skipPinba(data []byte) (n int, err error) {
	l := len(data)
	iNdEx := 0
	for iNdEx < l {
		var wire uint64
		...
		case 2:
			...
			iNdEx += length
			if length < 0 {
				return 0, ErrInvalidLengthPinba
			}
			return iNdEx, nil
		case 3:
			for {
...
var (
	ErrInvalidLengthPinba = fmt.Errorf("proto: negative length found during unmarshaling")
	ErrIntOverflowPinba   = fmt.Errorf("proto: integer overflow")
)
